...
const { sendSBB, info } = useSBB()

const durchsage = computed(() => `Arrival of the interregio 12 from ${info.value.from} to ${info.value.to} at platform ${info.value.platform}`)
const { isSupported, isPlaying, status, voiceInfo, utterance, error, stop, toggle, speak } = useSpeechSynthesis(durchsage, {
	lang: "en-US",
	pitch: 0.8,
	rate: 0.2,
	volume: 2,
